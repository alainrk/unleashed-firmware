...
### If your system is not added here that doesn't mean Flipper doesn't support it! Look into the add manually menu, and search for your manufacturer's instructions!
### Also many supported systems can be used only from `Read` mode. `Add Manually` is used only to make new remotes that can be bound with the receiver.
...
4. Hold Up arrow button on the Flipper to send programming signal - at the same time press and hold programming button on the receiver board.
5. LED on the receiver board will light on, then off, then on, then off again then on again
6. Release all buttons
7. Press send button on the Flipper a couple times holding it for 1-3 seconds
8. Done!
...
5. Follow manufacturer instructions on new remote programming

## AN-Motors AT4

**This instruction is for older boards. If yours has no** `Learn` **button but has buttons** `F`, `CL`, `+`, `-` **read instruction from Alutech AT4N**
...
5. Press `Send` on your Flipper again, LED on receiver will start flashing, wait a couple seconds until LED turns off.
6. Done
...
2. Open your new remote file
3. Open your receiver box, find button `F` press it for ~3 sec, display will show `Pr`.
4. Click `F` button a couple times until you see `Lr` on screen
...
7. Press button `F` on receiver board for ~3 sec to exit programming mode
8. Done
...
2. Open your new remote file
3. Push all 4 buttons at same time on your existing remote that already works with receiver
...
8. Receiver will beep again telling that new remote is added successfully!
9. Done!
...
Watch this video to learn more (videos in Russian language): https://www.youtube.com/watch?v=wZ5121HYv50 / https://www.youtube.com/watch?v=1ucrDKF3vWc
...
2. Open your new remote file
3. You need to be at minimum 3 meters from receiver
...
5. Long press (Right Arrow) - (0xF button - Btn:F) on Flipper for like 3-5 sec -> Will act like holding Button 1 & 2 on original remote as shown in video
6. Done!
...
For 4 button remote press & hold two buttons at upper row
...
9. Using counter values like `FF F9` can help bypass current original remote counter value, and as a result it also can fully desync original remote, only one remote can work at same time using this method
...
Now you will have exact clone of your remote that will have same counter, by making couple presses you will make it higher than original and receiver will work with it, but original remote will require same amount of presses to work again, and vice versa.
11. Also your original remote may become non working since it needs to be re-added into receiver board if you made counter much higher than original :(
...
8. Done, when using CAME Atomo from Flipper please hold Send button for at least 2 seconds to allow code to be fully transmitted, Flipper transmits only while button is held
...
6. Done, when using CAME Atomo from Flipper please hold Send button for at least 2 seconds to allow code to be fully transmitted, Flipper transmits only while button is held
...
an authorized remote control (note: the first remote control must always be entered
...
to enter) and OLD (the authorized one), position yourself within 3m of the gate/garage
receiver and then:
